...
setwd("~/Documents/Data/Annual Report/report_paragraphs/future_texts_paragraphs_futureonly")
...
#read in metadata
metadata<- read.csv(file="metadata_futureperfect_paragraphs_futureonly.csv", quote="", stringsAsFactors=FALSE)
...
tbl1<-tbl1 %>%
  dplyr::select(Filename, paragraph_filename, Year, text)%>%
  filter(Year>=1930)

#save docs
docs<-tbl1 %>% select(text, paragraph_filename)
#save(docs, file="doctext.Rdata")
...
save.image('/Users/carlyknight/Dropbox/PROJECTS/ConceptionsofRisk/data/stm/future_paragraphs_preparations_futureonly_7JULY2020.RData',
           compress = TRUE)
